...
- Re-picking later is `/profile/topics` — the same list in four tabs, saved on every toggle, floor of one (§8.1).
...
1. **Structural quality floor** (free, heuristic): drop items sharing a normalized title with >2 others (interchangeable catalog stubs — 0.4 found 67 items titled just "textile"), bare single-noun titles on image items, summaries under a minimum-signal bar. Items that fail are never inserted. **Two exemptions, both for walk sources.** Walk sources skip *dup-title* (09-01-26): a blog's series posts share a caption-derived title on purpose. Walk-source **images** additionally skip *bare-title* and *thin-summary* (09-06-26, `docs/PLAN_caption-less-and-wild.md`): those two rules are about museum *records*, where the text is all there is, and applying them to a picture blog let a museum rule decide which designated blogs could be ingested at all — thin-summary alone floored 137 of 150 `thevaultoftheatomicspaceage` posts and 140 of 140 `thisisnthappiness` posts, both of which then sampled at 8.45 and 8.11 average with the floor lifted. For a walk image the curator, which sees the picture, is the whole quality bar (§0's growth principle: everything that clears *quality*). Walk **articles** (pdr's) keep both rules — a 40-char article summary is still nothing to read — and search-shaped sources are untouched.
...
2. **Item pick** — within the chosen topic, weighted random over unseen items above the **curation-score floor** (default 4): `weight = (score − floor + 1)^power × (1 + boost per aesthetic_tag shared with the user's taste keywords)`. Never similarity-ranked — that was the 0.4 failure.
   ...
   >
   > **Cut 1 note (09-02-26; amended 09-06-26).** `getTopicPools` reads `item_topic` since 09-11-26 (the rail's `drawFromTopic` still reads `item.topic_id`). Consequences for an item that is still un-homed: an **un-homed** item (`topic_id IS NULL`) never enters a *topic* pool — it reaches the feed only as a **WILD** card (§9, 09-06-26), which is a real page slot but not a topic one; saving one bumps no topic (the toast reads only "Saved to X"); its `/i/` page has no wander teaser and its picture rail is all-wildcard. The gallery's wildcard draw (`drawImageAnywhere`) has no topic filter, so an un-homed *image* can surface in a wildcard slot — deliberate: it is curated-weighted like every draw, and ignoring the graph is the wildcard's job.
