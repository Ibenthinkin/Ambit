...
**Open:**
- **Multi-topic collisions at ingestion (decide in 3.4).** `item` is UNIQUE on `(source, source_id)` (§5.1) but `topic_id` is single-valued and NOT NULL — so when the same object answers two topics' seed queries, ingestion must pick one, and the rule is currently unspecified. This is not hypothetical: Phase 0's harvester deduped last-topic-wins, which silently starved whichever topics came *first* in its list. Measured on AIC, whose `/artworks/search` is a relevance ranking over the whole 132k-object corpus rather than a filter (so results overlap heavily across topics): `astronomy` found **419** usable items, **415** were claimed by later-ordered topics, **4** survived — matching the corpus exactly. Machines kept 1. Options: first-wins, highest-search-rank-wins, or letting an item carry a topic *set* (a schema change). Whatever 3.4 picks must be order-independent, and the ingestion log should surface collision counts so this can't recur invisibly. Knock-on: the Astronomy and Machines centroids in `topic-graph.json` were computed from AIC-starved samples — both still had 350+ curated items from the other four sources and their bridges read as sound, so it's a footnote to revisit after 3.4's real ingestion, not a blocker.
