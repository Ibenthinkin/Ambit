...
- **Playwright (e2e):** eight spec files, 42 tests, covering every flow this section originally listed. `auth.spec.ts` (5.2) — invite gating, sign-up, sign-in, sign-out, and the password-reset round trip through Mailpit's HTTP API. `feed.spec.ts` (5.6) — onboarding → a populated two-column feed, infinite scroll, long-press → item sheet → save, tap → item page → back with `?focus=`, the pill's sheets. `item.spec.ts` (5.7) and `gallery.spec.ts` (5.8) — the public `/i/[itemId]` reader and the immersive `/g/[itemId]`, both largely signed-out. `saved.spec.ts` (5.9), `settings.spec.ts` (5.10), `home.spec.ts` (1.2), and `pwa.prod.spec.ts` (5.11), which checks the offline/caching strategy against a real service worker.
...
- **`tile.loc.gov` rate-limits by IP, and the reader shares the user's IP, not ours (08-21-26, Phase 6.2)** — a 334-image LoC ingest tripped a sustained HTTP 429 with no `Retry-After` header and no published budget, still blocking every request twenty minutes later, from every User-Agent including a browser's. The ingest is easy to slow down. The *feed* is the open question: a page of LoC heroes hotlinks a dozen `tile.loc.gov` images from the reader's own connection, and there is no way to know from outside how close that comes to the limit. **This is the strongest evidence yet for BUILD_PLAN 7.3's proxy-with-cache** — stronger than AIC's, which is a referer rule a proxy trivially fixes; this one is a budget that a *cache* fixes and a proxy alone might make worse by concentrating every reader's requests on one IP. Recorded, not solved (Phase 6.2 decision 5). *Blog images (6.3) ride the existing `/api/img` proxy; the cache layer remains 7.3's.*
