...
- First sign-in → topic-chip grid, **four stages, one facet each** (09-10-26, `docs/DESIGN_topic-facets-and-personas.md`): Subject → Medium → Look → Place, over **every pickable topic** (`topic.facet IS NOT NULL` — 100 of the 104 today). Any stage may be passed with nothing picked; **the floor is three picks in total**, and one `topics.setMine` call at the end writes the union, so abandoning onboarding halfway leaves no rows. Superseded here: "v1 = the 16 graph-validated topics" (settled 07-17-26, when DRIFT/JUMP needed a graph row per topic and the validated graph covered 16). Cut 2a's graph rebuild covers the whole vocabulary, and grouping by facet is what made a hundred chips a screen instead of a wall.
- Re-picking later is `/profile/topics` — the same list as four facet sections on one page, each under its onboarding question, saved on every toggle, floor of one (§8.1).
...
- `/onboarding` — topic-chip grid (first sign-in; redirect here until topics chosen). Built Phase 5.3, **four stages since 09-10-26** (one facet each, over every pickable topic — §3.2), `OnboardingScreen` (`src/components/onboarding/`), persisted via one `topics.setMine` at the end; see `docs/PHASE5_WALKTHROUGH_5.3.md`.
