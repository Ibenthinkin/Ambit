...
- **Tiered topic-drift feed over a curated pool** (§9): topics connected by an offline embedding-derived adjacency graph decide _where_ to look; curated-weighted random decides _what_ to show. Validated in Phase 0.5.
...
A blog is designated because the _blog_ was judged worth having. Every post that clears the structural floor and the curator's quality bar is stored, whether or not Ambit currently has a topic that fits it. An item's source tags and the curator's aesthetic tags are always stored, and are the raw material from which new topics are proposed.
...
The short form: **topics are the vocabulary Ambit _asks_ with; tags are the vocabulary the world _answers_ in.** Museums are pulled by topic; blogs push their own vocabulary in. "Everything" means everything that clears **quality** — the structural floor and the curation score are untouched; it is _subject_ fit that stops being a reason to discard an item.
...
- First sign-in → topic-chip grid, **four stages, one facet each** (09-10-26, `docs/DESIGN_topic-facets-and-personas.md`): Subject → Medium → Look → Place. **The chips are umbrella groups since 09-25-26** (design §2a; `src/server/config/topic-groups.ts` — 12 Subject, 8 Medium, 8 Look, 6 Place), each a hand-assigned set of pickable topics (`topic.facet IS NOT NULL`, 159 today, every one in exactly one group); picking a group picks every member the server listed, and a group none of whose members exist is not shown. Any stage may be passed with nothing picked; **the floor is three picks in total** (chips tapped), and one `topics.setMine` call at the end writes the union of the members, so abandoning onboarding halfway leaves no rows. Groups are a picker-side idea: `setMine` receives topic ids and the feed never hears of them. Superseded here: "v1 = the 16 graph-validated topics" (settled 07-17-26, when DRIFT/JUMP needed a graph row per topic and the validated graph covered 16). Cut 2a's graph rebuild covers the whole vocabulary, and grouping by facet is what made a hundred chips a screen instead of a wall.
- Re-picking later is `/profile/topics` — the same groups as four facet sections on one page, each under its onboarding question, with the flat topic list behind a per-section "Show all" disclosure for fine tuning; a group chip is on / off / `mixed` by its members, saved on every toggle, floor of one (§8.1).
...
- Image card: tap → the item page, which _is_ the immersive picture screen since 09-10-26 (`docs/DESIGN_screen-structure.md`): the picture edge to edge at the top, square-cornered, with its facts below on scroll. On the picture, **sideways** walks the endless wander rail (§9) and the whole page — title, facts, address bar — follows; a **tap** toggles the caption and pill (a mouse moving over it summons them on desktop); a **quick downward flick** with the page at the top, or **Escape**, returns to the feed, popping the intact feed when the visit came from it. Upward movement is the browser's scroll. ←/→ page the rail. A long-press or right-click on a tile opens the item sheet — Closer Look, Share, and save, including making a new collection in place.
...
- Any item is viewable read-only at `/i/{itemId}` (public-domain content; no auth, no owner info). It is the one public page per work: `/g/{itemId}` (the Phase 5.8 gallery) redirects there since 09-10-26.
...
| `topic_id`                | the item's **display** topic — the seed query that surfaced a museum item, or the first honest topic the curator named for a walk item; `NULL` for an un-homed walk item (Cut 1). Full membership is `item_topic` (§5.1a). **Since 09-11-26 the feed's three TOPIC tiers draw on `item_topic` (§5.1a), not this column** — an item is drawable under any topic it is a member of, and this column is what the item page, the save toast and the wander rail show. An un-homed item has no membership row, so it reaches no topic pool. Since 09-06-26 the **WILD** tier (§9) is the one draw that asks for `NULL` on purpose, so an un-homed item is reachable without being force-fitted into a topic.                 |
...
| `license`                 | always an honest, non-empty string. For blogs the constant is truthful rather than permissive — _"Rights retained by original authors — displayed with credit and link"_. Never a fair-use claim.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `body`                    | **Settled 08-25-26 (6.3): `body` is `null` for every blog item, always.** The blurb is `summary` (the blog's own excerpt). Blog items are `type: "image"`, so the reader view is unreachable for them by construction; `source-invariants.test.ts` asserts it — both halves, iterating the designated blogs. **Widened 09-02-26 (PDR):** a walk source that is not a blog may carry a body — a Public Domain Review _collection_ is an image item whose `body` is its own CC BY-SA preamble, rendered on the item page under the picture (`image-item-body.tsx` → `ReaderBlocks`, introduced by `ReuseNotice`); a CC BY-SA _essay_ is an article. Full blog article text is still used at ingest only and never stored. |
| `(source, source_id)`     | unique → ingestion upserts idempotently.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| _(no `embedding` column)_ | per-item vectors exist only in the **offline pipeline** (dedupe, quality checks, topic-graph recomputes); the serving DB never stores or queries vectors. This is the Phase 0.4 pivot.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
CREATE TABLE topic (
  ...
  tier         TEXT NOT NULL DEFAULT 'original' -- 'original' | 'grown'  (Cut 2a; renamed 09-10-26)
  facet        TEXT                            -- 'subject'|'medium'|'look'|'place'  (09-10-26)
);
```

> **`tier` (Cut 2a, 09-02-26; `core` renamed `original` 09-10-26).** `original` is the sixteen Ambit shipped with: the config-defined, query-seeded topics whose adjacency was tuned by hand in Phase 0.5 and whose graph rows the rebuild preserves byte-for-byte. `grown` is vocabulary mined from the corpus's own tags and promoted by hand (`scripts/mine-topics.ts` proposes, Ben verdicts, `scripts/promote-topics.ts` applies). The word `core` was retired because it had started to imply a curated centre; the sixteen were the first words anyone thought of. A promoted topic carries `seed_queries {}`: it is vocabulary for classifying walk sources, not a query to send five museum APIs.
>
> **`facet` (09-10-26, `docs/DESIGN_topic-facets-and-personas.md`).** How the pickers group a topic, and — because `listTopics()` filters on it — whether a topic is pickable at all. `NULL` means not pickable: nobody has classified it yet (a fresh promotion), or it is an era topic (`19th-century`, tag-only since 09-07-26 — since 09-11-26 the feed draws on the membership join, so its pool is its tag memberships and giving it a facet is now a choice rather than a dead end). The authority is `src/server/config/topic-facets.ts`, applied to every row by `db:seed` on each boot, which is what puts facets on production with nothing copied into the container; `promote:topics` refuses to promote a topic without one. Both tiers are pickable now — the tier says where a topic came from, the facet says where it appears.

> The **topic adjacency graph** (each topic's row of neighbours ranked by similarity — §9) is _not_ a table: it's a checked-in JSON artifact (`server/config/topic-graph.json`, hand-editable, diffable in a PR). The feed reads it at startup. **It is a hybrid as of Cut 2a**: the original sixteen keep the Phase-0 sims computed from mean-centered topic centroids, because Ben tuned the drift feel against those exact values in Phase 0.5, and every edge touching a promoted topic is IDF-weighted tag co-occurrence **rescaled per row to the embedding graph's mean spread** — raw co-occurrence is ~4× flatter (−0.033..0.093 against −0.384..0.348) and `pickDrift` softmaxes over a row's positive head, so writing raw values in would soften DRIFT to a near-uniform draw. `scripts/rebuild-topic-graph.ts` regenerates it and refuses to write if a tuned edge changed. Co-occurrence was validated against the shipped graph before being trusted this far: mean Spearman ρ 0.502, top-3 neighbour overlap 50% — real signal, not a drop-in replacement, hence the split. At 99 topics the artifact is 647 KB / 9,702 edges; a `topic_edge` table becomes necessary around 300 (Cut 2b).
...
Collection _creation_ is a Phase 5.10 concern (it lives on the Profile screen in the design), so there is deliberately no `createCollection` procedure yet.
...
Every item a user has ever been served, retained forever (no TTL/pruning) — the feed's "almost never repeating" promise (§9). `served_at` is when the reader was actually _handed_ the item, set explicitly from the app clock rather than `DEFAULT now()`; as of Phase 5.7 the writer is the client's receipt ack (`feed.markSeen`), so it lands strictly _after_ the cursor `anchor` of the page it belongs to — which is exactly what keeps a refetch of that cursor from excluding its own page (§7's cursor design note). No separate `user_id` index: the composite primary key's btree already serves the feed's only query shape ("what has this user seen").
...
- **Corpus-walk sources and designated blogs (Phase 6.3, built 08-27-26; design `docs/PHASE6_DESIGN_6.3.md`, evidence `docs/PHASE6_WALKTHROUGH_6.3.md`).** A blog has no `search(q)`, so it is not a `SourceAdapter` — and that interface is a cross-service agreement ambit-archive implemented verbatim, so it was not changed. Instead a **sibling contract**, `CorpusWalkAdapter`, in the same `types.ts`: `{ source; walk(cursor?, opts?) → Promise<{ raw: Raw[]; next?: string }>; toItem(raw) → NormalizedItem }`. The cursor is opaque and adapter-defined; a 401/403 fails the walk immediately (`fetchJson`'s `noRetryOn`) — a blog that refuses us is a blog we stop asking. `WALK_SOURCES` in `config/topics.ts` is the walk half of `SourceId`; `config/blogs.ts` is the **designated-blog registry** (label, license string, robots policy) and `isBlogSource()`. Two registries, `adapters` and `walkers`, with complementary key types, so a source in both or neither is a compile error. **Ingest runs one lane per shape** (§6.4). Blog items are **link cards** (BUILD_PLAN 6.3's presentation contract): `type: "image"`, the post's featured image as hero, the blog's own excerpt as `summary`, `body` null always (§5.1), `attribution` = the blog, license _"Rights retained by original authors — displayed with credit and link"_, and on the item page a `LinkOutRow` — _Read the post on <blog>_ — under the blurb. **No fair-use claim**; removal on request is `--prune` (§6.4).
  - **`doorofperception` — Door of Perception**, the first walk source. WordPress with a live REST API (`/wp-json/wp/v2/posts?_embed&per_page=100`): four requests walk all 390 posts, slug as `sourceId`, `featured_media` as hero (a purpose-made ~800 px crop — tile- and hero-grade, not gallery-grade), `excerpt.rendered` (a written paragraph, not a truncation) decoded to text as the blurb. `robots.txt` is fetched and checked on every run; a `Disallow: /` aborts the walk and says so. **Topic assignment is the curator's classify mode**: the same call that scores the item also files it under one of the sixteen topics _or none_, and a null is dropped and counted rather than force-fit — a psychedelia post filed under `botany` would teach the drift graph something false. **Measured over all 390 before any write (08-27-26): 389 normalized · 3 floored (thin summary) · 318 classified (82%) · 68 honestly refused (18%)** — mythology 109, portraiture 59, botany 47, architecture 30, astronomy 17, machines 13, zoology 9, the-ocean 8, ancient-history 7, geology/music/typography 5 each, textiles 2, cartography 1, poetry 1, ceramics 0. Scores 8.64 avg, 92% ≥ 8 (the archive's doorofperception scrape, which D2 retires: 8.56 / 94%). The 68-post null bucket is the input to a later topic-expansion job, not a defect. **Scrape etiquette** (design §8): robots checked before designation and re-checked every run; honest `USER_AGENT`; ≥500 ms sequential per host; the ordinary ingest cron, no separate crawl; `--prune` for removed posts. **50watts.com was cut 08-25-26** — `User-agent: * / Disallow: /`, REST 403 regardless of UA — the artvee rule at the wire. Deliberately out of scope: a cache in front of `/api/img` (7.3), full-resolution heroes from the archive's `index.csv`, expanding the topic set, a suspended-items list.
  - **`thingsorganizedneatly` — Things Organized Neatly**, the second walk source and the first over Tumblr (built 09-01-26, `docs/HANDOFF_tumblr-walk.md`; **Keep — Ben's verdict 09-01-26, full walk written the same night**). Tumblr's legacy read API (`/api/read/json?start=N&num=50`, unauthenticated, answers `var tumblr_api_read = {…};` — `http.ts` gained `fetchTextResponse` for it), 5,522 posts, ~111 pages at the file's own `Crawl-delay: 1`. Two post shapes: `photo` (structured `photo-url-1280` + `photo-caption`; 162 of 200 sampled) and `regular` (the picture is an `<img>` inside `regular-body`; the largest `srcset` rendition is taken); every other type throws and is counted. Tumblr has no title field, so one is derived from the caption's first line (skipping a reblog's `name:` attribution), else the slug, else a placeholder that can never reach a reader. **Thin captions floor, never padded** — the "nothing to blurb" question is answered by doorofperception's own rule, not new machinery; the cost is a far higher floor rate (35% of the newest 150 under 60 chars, 60% across the archive). Numeric post `id` as `sourceId`, since the slug is empty on any captionless post. **Sample of the newest 150 (dry-run, 09-01-26): 83 curated @ 8.02 avg, 78% ≥ 8 · 55 classified (avg 8.18) into 14 of 16 topics · 28 refused (avg 7.71).** **Full walk (09-01-26, local): 5,267 offered · 25 rejected by `toItem` (video and answer posts, one imageless regular post) · 3,547 floored (thin-summary 3,320, bare-title 227, dup-title 0) · 1,720 curated · 891 written (avg 7.90, 71.8% ≥ 8, every row with an image) · 829 refused for having no honest topic.** Machines 311 and typography 174 lead; poetry 2. The 891 is below the sample's 1,300–1,500 projection because the archive thins with depth, as the sample warned. **Walk sources are exempt from the dup-title floor rule** (Ben's call, same day): a blog's series posts share a caption-derived title on purpose, where a museum's duplicated titles were interchangeable stubs. _`bare-title` and `thin-summary` applied too, until 09-06-26 exempted walk **images** from both (§6.2) — which reverses the "thin captions floor, never padded" sentence above for this source: its next walk stores the caption-less posts the 3,547-item floor dropped, and their titles are the blog's label rather than the placeholder._ One cosmetic finding for later: derived titles keep Tumblr prefixes such as `ed:` and `SUBMISSION:`. Two shared helpers grew: `decodeEntities` learned the named typographic entities Tumblr writes (`&rsquo;` etc.), test-first, after the adapter's HTML-safety test caught them.
  - **`thisiscolossal` — Colossal**, the third walk source and the first built on the **`wp-rest` factory** (`wpRestWalker(blog: BlogConfig)` in `sources/wp-rest.ts`, sources round 2, 09-01-26; **Keep — Ben's verdict 09-01-26, full walk written 09-02-26**). The factory carries doorofperception's verified facts (`_embed=wp:featuredmedia` with no `_fields`, `x-wp-totalpages` as walk length, 500 ms sequential, 401/403 refusal, missing-featured-image throw, `htmlToText` on title and excerpt); `doorofperception.ts` itself stays bespoke by Ben's choice, and the factory's test proves it byte-identical on that adapter's own fixture. One deliberate difference: tag names resolve **lazily per page** (`include=`, 100 ids a request, memoized per blog) because WordPress.com-hosted blogs carry tens of thousands of tags (mossandfog: 27,567). Canonical host `www.`; robots.txt carries a Cloudflare Content-Signal (`ai-train=no, use=reference`) and a managed named-bot list that does not name Ambit — recorded in `blogs.ts`. **Sample of the newest 150 (09-01-26): 139 curated @ 8.65, 97% ≥ 8, 116 classified into 13 topics, 77% would insert.** **Full walk (09-02-26, local): 8,818 offered over 89 pages · 0 `toItem` errors · 83 floored (all thin-summary) · 8,732 curated (3 curator errors: two heroes over OpenRouter's 30 MB image limit, one 502) · 6,075 written (avg 8.70, 97.5% ≥ 8, every row with an image) · 2,657 refused (30%) for having no honest topic.** All sixteen topics filled — architecture 933, zoology 826, portraiture 720, botany 622, mythology 583; poetry 35. Cost of a walk: Colossal serves ~2000 px originals, so a full curation pulls ~3 GB of heroes and takes ~38 min on the Mac; the walk alone is 8.5 min. **`mossandfog` — Moss & Fog** was built on the same factory the same night and **parked** by Ben's verdict (136 curated @ 6.60, 41% ≥ 8, an advertorial tail): it lives in `SUSPENDED_SOURCES`, the only switch that keeps a registered walker out of the nightly full ingest; no rows written.
  - **`pdr` — The Public Domain Review**, the fourth walk source and **the first that is NOT a designated blog** (sources round 2, 09-02-26; `docs/PLAN_publicdomainreview.md` is the design, **Keep — Ben's verdict 09-02-26, full walk the same night**). It is a publication whose featured images are public domain and whose own text it licenses **CC BY-SA 4.0**, so it gets its own `server/config/pdr.ts` rather than a `blogs.ts` row, and `blogs.test.ts` names it as the one deliberate exception to "every walk source is a blog". **Mechanism — a Gatsby build as an API:** publicdomainreview.org writes a `/page-data/<route>/page-data.json` beside every route, so four one-shot index requests hand over the whole archive (1,255 Collections · 343 Essays · Conjectures 21 · Curator's Choice 29 — the two series are _not_ in the essays index). The cursor is `<phase>:<offset>` over those four phases. **The cache is load-bearing:** every detail response is 0.5–1.2 MB of which 8–30 KB is the record, so an uncached full walk is ~1.7 GB from a non-profit's CDN; each hydrated record is written to `.cache/pdr/<kind>/<slug>.json` once and read from disk forever after (~22 MB), making a nightly walk four index fetches plus the new pieces. No revalidation — Netlify's ETags change on every detail whenever anything is published. **Rights policy lives in `walk()`, not `toItem()`:** a collection whose digital copy any source marks `Non-commercial` (Bibliothèque nationale de France, in practice) never becomes a raw, so it neither eats `--quota` nor prints a failure line nightly. **Two projections:** a Collection is an `image` item that _also_ carries its Preamble as `body`, rendered on the item page under the picture (`ReaderBlocks`, introduced by `ReuseNotice` — the first image items in the corpus with a body, which is why SPEC §5.1's `body` row and `source-invariants.test.ts` were both rescoped to _blogs_); an Essay is an `article` when `Publication_Rights === "CC-BY-SA"` and a **link card** otherwise (book excerpts and unlabelled guest posts — never reproduced). `LinkOutRow` learned `pdr` with its own copy. **Deliberately out:** the PDR _Blog_ (76 org-news posts, no images), footnotes, bibliographies, and the inline `{image}` blocks with their captions — the link-out reaches all of it. **Trial samples (09-02-26):** newest 150 collections **8.73 avg, 97% ≥ 8, 15/16 topics**; newest 60 essays **8.32, 95% ≥ 8**; both series **8.02, 72% ≥ 8**. **Full walk (09-02-26, local): 1,648 records in four indexes → 19 excluded on rights → 2 `toItem` errors (the only two collections with no featured image, `hands-1944` and `a-midsummer-schottische`) → 1,627 offered → 3 floored (1 bare-title, 2 thin-summary) → **1,624 rows @ 8.39 avg, 87% ≥ 8** (collections 1,231 @ 8.50 · essays-as-articles 318 @ 7.93 · essays-as-cards 75 @ 8.63). 1,547 rows carry a body. All sixteen topics filled — mythology 589, portraiture 217, ancient-history 232, typography 171; ceramics 16. **186 rows (11%) are un-homed** under Cut 1: stored, invisible to the feed until Cut 2's promotion path, and the best vocabulary material in the corpus — PDR is the only source carrying _controlled_ taxonomies (`Medium`, `Theme`, `Style`, `Epoch` plus curated tags) on every record. **One trap found the hard way:** exactly one slug in 1,648 carries a trailing slash (`…make-lemonade/`) — the record's own `Slug` field, so it reached the cache path, the detail URL and the `sourceId` at once and killed the first full walk at cursor `e:100`, before either series was walked. `slugOf()` normalises at the wire boundary; `cachePath()`'s guard stays an absolute assertion and was deliberately NOT loosened to accept it.
  - **`loupe` — Loupe**, the fifth walk source and **the second that is not a designated blog** (Loupe Phase 4; `docs/PLAN_loupe-hookup.md`, **built 09-07-26 on both sides**). Loupe is Ben's own magazine-clipping bench (the `loupe` repo; Ambit-Admin's Ecosystem Architecture calls this the corpus-walk pattern's canonical case): Internet Archive scans of the Whole Earth Catalog segmented into articles and illustrations and triaged by hand, of which `GET /api/v1/articles` serves only what he **kept**. It gets `server/config/loupe.ts` rather than a `blogs.ts` row, and `blogs.test.ts` names it beside `pdr` as a deliberate exception. **Mechanism:** `GET $LOUPE_URL/api/v1/articles?limit=200&cursor=…` under `Authorization: Bearer $LOUPE_API_TOKEN`, oldest issue first over a keyset cursor Ambit passes back opaque (Loupe SPEC §8.2); a 401/403 ends the walk on the first response (`noRetryOn`). Today's corpus is one request. **Identity is position, never Loupe's `id`:** Loupe deletes and re-inserts a page's articles on every correction, so `sourceId` is `<iaIdentifier>:<pageNumber>:<readingOrder>` — a corrected clipping upserts in place, a vanished position is removed by `--prune` on a complete walk, and a corrected clipping keeps its old score until a `PROMPT_VERSION` bump (the curation cache is keyed on `source:sourceId`). `readingOrder` joined Loupe's wire for exactly this (Loupe `dc55380`). **The bearer rides on both server-side image fetches** — the curator's download at ingest and the proxy's cache fill — via `services/image-auth.ts`, decided **by source** (`imageFetchHeaders("loupe")`; every other source merges `{}`), because Loupe's `/media/*` is gated the same way as its API and an adapter without that half would curate blind and fill the proxy with 401s. **Rights posture:** the license string is Loupe's own, stored verbatim (_"No open license — rights retained by original authors; personal use only."_), `body` is stored for articles (the OCR text, typeset by `ReaderBlocks`) and null for illustrations, tags are Loupe's tags only, and **there is no per-user gate, by Ben's 09-06-26 decision** — Ambit is invite-only and every reader is someone he knows; `item.source` is the column a future gate filters on (feed pool, public item resolver, image proxy — three filters, not a migration). No robots check and no politeness delay: a service on Ben's own machine that exists to be walked. **First walk (09-07-26, local, against the 99-topic classifier):** 133 offered → 1 floored (thin-summary, an article) → 132 curated @ **4.52 avg, 5% ≥ 8** (histogram 1:11 · 2:11 · 4:69 · 6:8 · 7:26 · 8:6 · 9:1) → 112 classified into 29 topics (science 45, technology 42, machines 34, illustration 30, advertising 17), 20 un-homed (technical diagram, building-construction-reference, diagram, personal-narrative). Rows written: **131 articles + 1 image**, every article with a body, 110 of 132 above the feed's `scoreFloor` of 4. Image-fetch failures 0 at ingest; `img:warm` filled 130 + 2 cached, 0 failures. The low average is honest and Loupe-shaped, not a bearer fault: a clipping is a region of a catalog page, its OCR often starts and ends mid-sentence, and the curator judges an _article_ from its text — the one _image_ item scored 9. Thirteen short clippings carry a Loupe summary identical to their body, so the item page shows the text twice; a Loupe-side nit, not Ambit's. **Production is not covered:** Loupe has no production host, so `loupe` belongs in `SUSPENDED_SOURCES` on the branch that next deploys, or the nightly ingest prints a "not configured" error for it every night.
  - _Superseded 08-27-26 — the original 08-20-26 strategy note, kept for the record:_ Ambit's content is to come primarily from **designated blogs**, which already carry the tags, descriptions and articles that image sources make you manufacture. It is an **in-repo adapter family** — a shared scraper core plus per-blog config, one `SourceId` per blog — and deliberately **not** a third cross-service integration pattern; the ecosystem's two blessed patterns govern seams _between_ services, and this stays inside Ambit. Its presentation contract is **excerpt + link-out**: image or short excerpt, Ambit's own description, a 1–2 sentence blurb about the article, a visible `from: <blog>` credit and a prominent link to the original. **Ambit hosts no reformatted articles** and makes **no fair-use claim**; full text is used at ingest only. Blogs do not `search(q)`, so the adapter interface above does not fit as written — that and six other questions are open. See `docs/BUILD_PLAN.md` step 6.3 and, for the governing decision, Ambit-Admin's `Ecosystem Architecture.md` (08-20-26). Designated-blog list: [`docs/source-candidates.md`](docs/source-candidates.md).
- **Phase 6.2 promoted three more, the first round of the trial loop to run** (08-21-26; evidence and Ben's verdicts in `docs/PHASE6_WALKTHROUGH_6.2.md`). Four were trialed, three kept, one parked. This is what "promote one into this list only after it passes the trial" looks like in practice — the entries below record the quirks _found_, not the ones expected:
  - **`smithsonian` — Smithsonian Open Access** (api.si.edu). Keyed: `SMITHSONIAN_API_KEY`, a free api.data.gov key, **optional** for the same reason the archive's pair is (ingest-only; a clone without one must still boot and ingest everything else), read at `search()` time. 1,000 requests/hr on a real key versus DEMO_KEY's 10. Licensing is the friendliest of any source here — `media_usage:"CC0"` is a _query_ filter, and 400 of 400 sampled rows carried `usage.access: "CC0"`, so unlike the Met there is no per-item second call — **but the filter is not sufficient alone**: 2 of those 400 simultaneously carried `indexedStructured.online_media_rights: ["Copyright protected/restricted"]`, so both signals are re-checked and contradictions dropped. The rule deliberately spares the far more common _permissive_ value in that field, "No Known Copyright Restrictions". `sourceId` is `record_ID` (unit code + accession number), never the row's own timestamp-shaped `id`, which a re-index would be free to change — taking the corpus with it. `imageUrl` gets `&max=1200`: the IDS delivery service defaults to the full-resolution JPEG (837KB versus 89KB capped). **The quirk to plan around is density, not rights** — the natural-history catalogues dominate by row count, and 31% of offered items are dropped by the structural floor before any LLM call (dup-title mostly). Seed vocabulary carries the weight there: `specimen` returns 5,029,697 rows and is never used; `textile` loses most of its yield to objects literally titled "Textile", so the cell asks for `embroidery` and `lace` too. 14 of 16 topics; poetry is the honest omission.
  - **`loc` — Library of Congress**, scoped to cleared collections. No auth; `?fo=json` on any `/pictures/search/` URL returns clean JSON and every result carries a ready full-size image on `tile.loc.gov`. **The per-result `rights` field is empty on every row**, and the Library holds a great deal of in-copyright material, so the adapter never searches all of loc.gov: `CLEARED_COLLECTIONS` lists collections the Library has blanket-cleared, each with its rights statement recorded **verbatim** from the API's own `rights_information` — for the Margolies archive, "No known restrictions on publication", deliberately _not_ upgraded to "public domain", which is a stronger claim than the Library makes. Scoping is `q=<token> <query>` plus a re-check of each result's own `collection[]` membership. **The growth path is the point of the entry**: one verified rights statement at a time, a new `CLEARED_COLLECTIONS` row per collection, no adapter change. First and so far only entry: `mrg`, the John Margolies Roadside America photograph archive (11,708 images). Best curation scores in the corpus at sample scale (8.52 avg, every item ≥8; 7.98 across the full 376). **`tile.loc.gov` rate-limits by IP with no `Retry-After` and no published budget** — a 334-image ingest tripped a sustained 429 that outlasted the run. _Solved in 7.3 (08-28-26): the image proxy caches to disk, so each image is fetched once ever, and `bun run img:warm --source loc --rate 1` filled 372 of them with no 429 at all — the 6.2 failure was the burst, not the budget. See **Image delivery** in §8._
  - **`nasa-images` — NASA Image & Video Library** (images-api.nasa.gov). No auth. Chosen over APOD, which earlier drafts of BUILD_PLAN 6.2 named: the whole media catalogue rather than one keyed image a day. **The licensing posture is the weakest of the three and is stated rather than filtered**: a 600-item survey found _no rights field of any kind_ anywhere in the response, so there is nothing to filter on and nothing to re-check. `license` is the literal `"Public domain (NASA)"`, scoped by that word, and `attribution` reproduces NASA's own credit line verbatim (`secondary_creator` on 172/600, `photographer` on 291, originating center otherwise) rather than flattening every item to the agency name; a credit that doesn't already say NASA gets prefixed. Exactly 2 of 600 credited a non-NASA party. No image-URL rewrite: every item publishes its renditions as explicit `links[]` entries with widths, so the adapter picks down a ladder (`~medium` → `~large` → `~small` → `~thumb` → `~orig`) rather than guessing at a URL shape. Six topics.
  - **`poetrydb` — parked, not cut.** Its adapter and tests stay in the repo and its id stays in `TRIAL_SOURCES`; what it has is **no seed cells**, so ingest never reaches it (`seedQueries[sourceId] ?? []` skips an empty list). Two reasons, both recorded and both fixable: its summaries take the poem's first two lines and PoetryDB's `lines[]` includes epigraphs and dedications, so a feed card can lead with transliterated Greek; and the curator's rubric is written for images and cannot score a lyric poem — Pope and Seeger scored 4 against "visually striking… huh, I never knew that", which is the same structural reason Wikipedia averages 5.27. Un-parking it means giving it cells again and nothing else. Its API quirk is worth keeping even so: `GET /lines/<keyword>` **503s at any real result-set size** (nine keywords tested; only a single-poem match returned 200), so the adapter discovers with `/lines/<kw>/title,author` and hydrates each poem individually — and a no-match is a JSON _object_ at HTTP 200, not an empty array.
...
1. **Structural quality floor** (free, heuristic): drop items sharing a normalized title with >2 others (interchangeable catalog stubs — 0.4 found 67 items titled just "textile"), bare single-noun titles on image items, summaries under a minimum-signal bar. Items that fail are never inserted. **Two exemptions, both for walk sources.** Walk sources skip _dup-title_ (09-01-26): a blog's series posts share a caption-derived title on purpose. Walk-source **images** additionally skip _bare-title_ and _thin-summary_ (09-06-26, `docs/PLAN_caption-less-and-wild.md`): those two rules are about museum _records_, where the text is all there is, and applying them to a picture blog let a museum rule decide which designated blogs could be ingested at all — thin-summary alone floored 137 of 150 `thevaultoftheatomicspaceage` posts and 140 of 140 `thisisnthappiness` posts, both of which then sampled at 8.45 and 8.11 average with the floor lifted. For a walk image the curator, which sees the picture, is the whole quality bar (§0's growth principle: everything that clears _quality_). Walk **articles** (pdr's) keep both rules — a 40-char article summary is still nothing to read — and search-shaped sources are untouched.
2. **LLM curator** (cheap vision model via OpenRouter, `OPENROUTER_API_KEY`): a Tumblr-art-blog-curator persona scores each survivor 1–10 for visual/intellectual interest and writes 2–4 `aesthetic_tags`. **Image items are judged by the image itself** — download the image and pass base64; never hand a museum image URL to a third-party service (they bot-block server-side fetchers — bitten twice in Phase 0.5). ~1,450 tokens/item ≈ $1 per full corpus.
   ...
   - A source may name a **smaller rendition for scoring only** (`NormalizedItem.curationImageUrl`, 09-06-26 — additive and optional, so the cross-service `SourceAdapter` contract is unchanged and no existing adapter sets it). The Tumblr walker points it at `photo-url-500`; a 1-10 score and four tags do not improve above ~500 px, and it cuts the four budgeted Tumblr walks' curator download from ~58 GB to ~12 GB. `imageUrl` remains what is stored and shown.
...
**Classify mode (walk sources) — Cut 1, 09-02-26.** Returns an **array** of honest topics, best fit first, "usually one or two, never more than three", possibly **empty**; only ids in the vocabulary offered survive parsing. **The vocabulary is every topic in the database, not the compile-time sixteen (09-06-26).** `classifyPrompt(topics)` takes the list and ingest passes `listAllTopics()` — 99 today — so a walk item whose subject is one of Cut 2a's grown topics homes at ingest instead of waiting for the next manual `promote:topics`. On the four Tumblr blogs sampled that day this took un-homed from 40-44% to 0-3%. It re-bills nothing: the cache key has no topic-list input by design, so an item classified under sixteen keeps that answer and promotion remains its backfill. Nothing is dropped for subject fit — ingest stores every curated walk item (§6.4). No item was re-billed: `PROMPT_VERSION` stayed at 1, the `classify|` cache namespace was reused, and a Phase 6.3 entry reads forward (`topicId: "botany"` → `["botany"]`, `null` → `[]`); items curated before Cut 1 keep their single topic until a Cut 2 promotion widens them. The cache key is deliberately not a function of the topic list. `docs/DESIGN_topic-vocabulary-growth.md` §6.

**Parked experiment — visual embeddings** (Voyage `voyage-multimodal-3.5`, prototyped in `phase0/embed-images.ts`): image vectors capture _form/vibe_ where text vectors capture _subject_ — potentially a "more like this look" affordance on saves. Keep-or-cut verdict pending Ben's blind-harness browse (§15).
...
- `saves.ts` — `unsaveItem`, `isItemSaved`, `getSavedItems(userId, { collectionId? })`, `getSavedCount(userId)`. Reads and the delete only: the _write_ path is `collections.ts`'s `setItemCollection`, because every save in the redesign goes through the save-to-collection sheet and therefore always carries a collection.
...
| `topics.resetWeights`    | mutation | —                                                                                                                   | `{ reset: number }` — **dev-only** (same gate); every `user_topic.weight` back to 1.0                                                                                       |
...
| `saves.collections`      | query    | —                                                                                                                   | `{ id, name, createdAt, itemCount, covers }[]` — `covers` the proxied srcs (`/api/img/<itemId>`) of the four newest saved pictures, newest first, `[]` when none (09-12-26) |
| `saves.saveToCollection` | mutation | `{ itemId: string, collectionId: string, topicId?: string }`                                                        | `{ collectionName: string, drift: { topicLabel, isNew } \| null }`                                                                                                          |
...
- `topicId` (09-11-26) is the topic the card was served under; the server bumps it only if the item is a member of that topic (`isItemInTopic`), else the display topic — a client cannot bump an arbitrary topic. `docs/DESIGN_chrome-redesign.md` §5.
...
- `feed.page` returns `cards`, not bare items (revised at Phase 4.1 build time — see below): each
  `FeedCard` is `{ item: Item, tier: "CORE" | "DRIFT" | "JUMP" | "WILD", topicId: string | null, driftPath?:
...
- `feed.page`'s `knobs` input (Phase 4.2) is zod-bounded to a sane range per field (mirroring
  ...
  `topics.weights`, `topics.resetWeights`
...
- **`feed.forgetSince` (dev knob panel, 09-05-26)** is the un-burn behind `/dev/feed`: it deletes
  ...
  a tuning session must _ack_ like production — the cursor's `anchor` moves to each page's
...
- **Three public (unauthenticated-allowed) procedures**, backing the app's one public route
  `/i/{itemId}` (`/g/{itemId}` redirects to it since 09-10-26): `items.byId`; `items.wanderNext`
  (Phase 5.7), the item page's "where Ambit would wander next" teaser; and `items.galleryRail`
  (Phase 5.8), the endless rail the item page's picture swipes through. All three are safe to expose by construction rather than by care: none
...
source, sourceUrl, license, topicId, topicLabel, debug?: { via, topic } }` — `body` and
  `topicLabel` since 09-10-26, because the merged item screen renders whichever cell is current
  whole (a PDR essay under its picture; the Topic fact, joined from `topic.label` in one query per
  batch, since the config knows only the sixteen originals); `debug` gated by `FEED_DEBUG`, as
  `feed.page`'s is. `count` is capped at 16 and `exclude` at 200, which bounds the SQL `IN`-list
  ...
  `seen_item` rows, ever**: swiping the rail is free, which is precisely why the rail is its own
...
`page` reseed the page's two RNG streams deterministically (topic stream `mulberry32(hashSeed(\`${seed}:${page}\`))`,
item stream the same key plus `:items`— two since 09-11-26, §9.2), so
refetching the same cursor against unchanged pool state reproduces the exact same page — the
actual mechanism behind "stable pages on refetch." Within one build; a deploy that changes the
streams or the pools composes a fresh page for a pre-deploy cursor.`anchor`is the ISO timestamp of the *previous*
...
_Anchor arithmetic after the Phase 5.7 receipt move._ Through 5.6 the server marked items seen
...
acks are _later_ than that anchor — so the page does not exclude itself and reproduces identically,
...
caller's IP taken from the _last_ `X-Forwarded-For` hop (the one segment a single trusted reverse
...
- `/` — landing + sign-in / sign-up (email + password; forgot-password link). **Rebuilt in 8.3** (`docs/DESIGN_landing-redo.md`, plan `docs/PLAN_landing-redo.md`), replacing 5.11's eight committed JPEGs: an **overture** on black — `AMBIT — A quieter way to be curious.` in Inter, the tail collapsing into the wordmark on the reference's clock (1.4 s hold, 2.2 s collapse, `clip-path` + `transform` so CLS stays 0) — then a hard cut into a **reel of the corpus's own pictures**, which resolves into the persistent `AuthSheet`. The reel is **picked by the server** (`services/landing-pool.ts`): every image at `curation_score ≥ 9` under an exact landing licence (`config/landing-pool.ts` — CC0, Public Domain Mark, NASA PD, LoC's "no known restrictions", PDR's `Public domain —` variants; ~2,700 pictures) **with a measured master** (`item.image_width/height`, written by `img:warm` and backfilled by `img:dims`, which runs non-fatally on every boot), memoised in-process for ten minutes (an empty pool is never memoised). Since 09-25-26 there are **two reels per visit**: **tall** pictures (0.4 ≤ w/h ≤ 0.8) for a phone held upright and **wide** ones (1.25 ≤ w/h ≤ 2) for anything else, each with a long edge ≥ 800 px — the screen picks by `(orientation: portrait)`, starting from the server's user-agent guess (`guessShape`) so hydration agrees. Twelve per reel; the guessed reel's first `gateFrames` are preloaded (an HTTP `Link` header); a shape with no pictures shows the one committed `public/landing/fallback.webp`. Pictures are the **master** — a full-bleed picture needs every pixel it has. **Two tempos** (`components/landing/tempos.ts`): `cut` (350 ms hard cuts, 12 frames, then the sheet — under WCAG 2.3.1's three flashes a second) and `dissolve` (6 s frames, 2.5 s fades, a slow 1.06 drift, 2 frames); behind the sheet either relaxes to the dissolve. `DEFAULT_TEMPO` is what production runs; `?tempo=cut|dissolve` overrides it **only under `feedDebugEnabled()`**. The wordmark stays over the reel in `mix-blend-mode: difference` until the sheet rises. Tap = next, ←/→ step, the floating glyph ("Open sign-in") skips, the sheet's disc ("Back to the slideshow") collapses and replays from the top. **The sheet is mounted from first paint and merely translated off-screen**, which is what `waitForHydration(page, "form")` and every auth e2e selector depend on. Inside it is 5.2's mode-toggle `AuthCard`, unchanged. `prefers-reduced-motion` skips the overture and shows one still with the sheet up; `Save-Data` plays the overture and holds the first picture.
...
- `/onboarding` — topic-chip grid (first sign-in; redirect here until topics chosen). Built Phase 5.3, **four stages since 09-10-26** (one facet each; umbrella group chips since 09-25-26 — §3.2), `OnboardingScreen` (`src/components/onboarding/`), persisted via one `topics.setMine` at the end; see `docs/PHASE5_WALKTHROUGH_5.3.md`.
- `/profile` is a hub (09-12-26, `docs/DESIGN_list-screens.md`): Collections · Topics · Edit profile · Settings as routed tabs under one layout (`app/profile/layout.tsx` → `ProfileHub`), each tab a `<Link replace>` so the hub is one history entry; `/settings` is a permanent redirect to `/profile/settings`, which is sign-out's home. A collection's face is a square-cornered 2×2 of its four newest pictures.
- `/profile/topics` — the topic manager (09-10-26): every toggle saved at once via `topics.setMine` (floor of one). A tab of the Profile hub since 09-12-26, also reached from Settings' "What you see" (a bottom sheet until 09-10-26); the four facets are chips. Under `FEED_DEBUG` each pressed chip shows its learned weight and a **Reset weights** button flattens them.
- `/feed` — the infinite feed (auth-gated, default authenticated landing). Built Phase 5.6 (`docs/PHASE5_WALKTHROUGH_5.6.md`), replacing 5.2's placeholder: an **RSC shell** — the two guards (session → `/`; not-onboarded → `/onboarding`, carried forward verbatim from 5.3), a `prefetchInfinite` + `HydrateClient` handoff, and `FeedScreen`. The route stays **dynamic** by construction (it reads `headers()` and the feed is per-user). The prefetch input and the client `useInfiniteQuery` input must stay byte-identical (`{}` on both sides) or hydration silently misses and the client refetches — which costs a page of the user's corpus, since a received page is acked seen (`feed.markSeen`, Phase 5.7). On a hover-capable fine pointer each tile carries a hover strip (last-used collection + one-click save; `docs/DESIGN_chrome-redesign.md` §3).
- `/saved` — saved items.
- `/i/[itemId]` — public read-only single item. Built Phase 5.7 (`docs/PHASE5_WALKTHROUGH_5.7.md`), replacing 5.6's interim stub. An **image variant** and a **reader variant** keyed on `item.type` — since 09-10-26 the image variant is the **merged item screen**, described under `/g/[itemId]` below, which it absorbed; a `from: <source>` credit line under both titles, linking `item.sourceUrl` (every source, not just blogs — §6.1's rights posture, generalized); the article body typeset from the stored `body` column via `src/lib/reader-blocks.ts` (no runtime source fetch, no source HTML, so nothing to sanitize); a `?from=Name` param-driven "shared this with you" row (≤40 chars, text-only, never persisted, never looked up); the `items.wanderNext` teaser on both variants; and a join CTA for signed-out visitors only. The pill toolbar, both sheets, and the protected `saves.forItem` query render **only** when authed — a signed-out visitor triggers no user-scoped request at all, and `generateMetadata` is built purely from the item row so a shared link's preview carries nothing about the sharer. Leaving is `useLeaveToFeed`: **pop** history when the visit came from the feed, push `/feed?focus={id}` only for a cold-opened link — a pushed navigation re-runs the dynamic `/feed` and draws a fresh page of corpus. On the reader a horizontal swipe-back (`useSwipeBack`, 0.35× rubber-band follow, commits past 70px) and Escape share that same exit; on a picture the exits are Escape, a quick downward flick at the top of the page, and the pill's Feed button. Ungated in `src/proxy.ts` by design.
- `/g/[itemId]` — **a permanent redirect to `/i/[itemId]` since 09-10-26** (`docs/DESIGN_screen-structure.md`), so no bookmark or history entry 404s. It was the immersive gallery (Phase 5.8, `docs/PHASE5_WALKTHROUGH_5.8.md`), entered by tapping the item page's hero; the two were rooms for one work and are now one. **The merged screen** (`components/item/item-screen.tsx`): `HeroRail` at the very top — the gallery's three-cell track, `touch-action: pan-y`, square-cornered and top-aligned, its height following the loaded picture on the phone (`min(viewport, width × ratio)`, the full viewport until the ratio is known) and the full viewport height above `md`, with the caption and pill _below_ a short picture — in a row that collapses to nothing while hidden, so a hidden caption leaves no band — or overlaid on a full-height one — then `ItemFacts` in the 720px reader column (title, maker, credit, summary, a Maker / From / License / Topic table with absent facts omitted, a PDR body, the blog link-out), the wander teaser (a client query keyed on the current cell, the entry's answer handed in as `initialData` so a cold link still costs no client request), and the join card for signed-out visitors. Swiping walks the wander rail (§9) and marks nothing seen; on advance the URL is `history.replaceState`d to `/i/{current}` and the document title with it, so the address bar, a reload and Share all name what is on screen. The chrome starts hidden and cycles on a 10s loop; a tap toggles it, a _mouse_ moving summons it (a pointer event filtered to `pointerType === "mouse"` — a tap's compatibility `mousemove` must not undo the tap). There is no details sheet. Every exit is `useLeaveToFeed(entryItemId)` — Escape, a quick downward flick with the page at the top (`overscroll-behavior-y: contain`, so it is never also a pull-to-refresh), the pill's Feed — so after any number of swipes Back still pops to the intact feed. Saved's image tiles open `/i/` too. The pill, both sheets and `saves.forItem` render only when authed, as before.
- `/api/img/[itemId]` — the image proxy (Phase 5.7). Takes an **item id and looks the URL up in our own table; it never accepts a URL**, from a query param or anywhere else — that is the SSRF/open-proxy boundary. It fetches server-side with Ambit's UA and **no `Referer`** — which removes the variable behind the Art Institute of Chicago's `localhost`-referer block (`docs/HANDOFF_aic-images.md` §2.2), though AIC turned out to have escalated to a host-wide Cloudflare _interactive challenge_ that no server-side fetch can pass, so that source stays suspended (ibid. §8). Successes stream through with `Cache-Control: public, max-age=31536000, immutable`; every failure path is `no-store`. Its rate limiter is a **separate, generous instance** (600/min per IP) rather than the shared tRPC one — a single feed page loads ~24 images and would otherwise starve the API. `data:` image URLs bypass it client-side. Resizing / IIIF sizing / a CDN cache layer are deliberately deferred to 7.3.
...
(a _referer_ problem, which a bare proxy fixes), and `tile.loc.gov` rate-limits **by IP with no
published budget** (a _budget_ problem, which only a cache fixes — a bare proxy would concentrate
...
- **One variant per item** (D2): ≤1600 px on the longest edge, WebP quality 82, EXIF-rotated, never
  enlarged. 1600 covers a 3× phone at the hero's rendered width and the item screen's edge-to-edge picture.
- **One rendition since 8.3** (`docs/DESIGN_landing-redo.md` D3): `?w=960` serves a ≤960 px WebP at
  quality 76 (measured p50 45 KB / p90 132 KB vs the master's 78 / 263), stored as
  `${itemId}.w960.webp`. It is **derived from the cached master, never fetched upstream on its own**
  (a missing master is filled first, once, as ever), and the set is **closed** — `RENDITIONS = [960]`;
  any other `w` is a 400 `no-store` before any cache work, so the route never takes a caller-chosen
  size. Nothing uses it since 09-25-26 (the landing reel moved to masters); it stays, closed and tested, for the next thumbnail-shaped need. `bun run img:warm --rendition 960 --landing` pre-derives
  the landing pool (CPU only; `.cache/landing-prod.sh` runs it in the production container).
...
Implementation: `src/server/services/image-cache.ts` (`getOrFill` / `fillCache` / `readCached`,
and `getOrFillRendition` since 8.3), a
thin `src/app/api/img/[itemId]/route.ts`, `scripts/warm-images.ts`, `IMAGE_CACHE_DIR` in `src/env.js`.

### 8.2 Components

- `components/feed/` (Phase 5.6) — the built names, which diverge from the sketch below. `feed-screen.tsx` is the infinite scroll (`useInfiniteQuery` on `feed.page`, `IntersectionObserver` sentinel rooted on the **viewport**, 500px margin — and since 09-10-26 a **re-check** that measures the sentinel whenever a page lands or the query learns of a next page, because an observer calls back only on a crossing: a first page too short to carry the sentinel out of the margin produced none, so page 2 never loaded, a scroll to the bottom never appended, and the next remount drew it instead; the re-check is skipped after a failed next page so it can never become a retry loop); `masonry.ts` holds the layout decisions as pure functions (`buildTiles` inserts at most one serendipity tile per fetched page, on its first `JUMP` with `driftPath.length >= 2`; `packColumns` greedily fills the shorter of two columns, which — unlike CSS `columns` — can never reorder a tile the reader is already looking at); `image-tile.tsx` / `article-card.tsx` / `because-tile.tsx` are the three tiles; `use-feed-scroll.ts` is the `?focus=` return-scroll and session scroll restore.
- ~~`FullscreenGallery.tsx`~~ — built as `components/gallery/gallery-screen.tsx` (5.8) and **retired 09-10-26**: the picture screen is `components/item/item-screen.tsx` (`hero-rail.tsx` + `item-facts.tsx`), its gestures `hooks/use-rail-gestures.ts` and its chrome cycle `hooks/use-chrome-cycle.ts`.
- `components/sheets/collection-rows.tsx` — the collection-row family and **`NewCollectionRow`**, the app's one create form (09-10-26): a row that becomes a name field. Every collection picker ends with it — the tile's item sheet (which also gained Share), the item screen's save sheet (both file the item into what they make), and the pill's Collections sheet (which opens the new collection's Saved list) — and the profile's New-collection sheet is just it, expanded.
...
1. **Tier draw** — default mix **CORE 40 / DRIFT 35 / JUMP 25 / WILD 10** (drift-heavy per Ben's 0.5 verdict: "what I enjoy most is the higher, further drift"). **WILD** (09-06-26, `docs/PLAN_caption-less-and-wild.md`) is the fourth tier and the only one with no topic behind it: roughly one card in twelve, drawn from the **un-homed pool** — curated items `topic_id IS NULL`, which no other draw can reach — weighted only by overlap with the reader's recent saves' aesthetic tags (`wildTagBoost`, 1.0, since with no topic it is the slot's one signal). It is **not** the gallery rail's wildcard: that one is public, unpersonalized and ignores the un-homed distinction; WILD is personalized, draws only un-homed items, and burns `seen_item` rows like every feed tier. Its pool is a deterministic 200-row SQL sample (`ORDER BY md5(id || '<seed>:<page>')`) so a cursor still reproduces its page. `tierWild: 0` skips the query entirely and composes exactly as before. Two levers on how far the walk reaches into the grown tier, both `1` by default: `grownEdgeScale` (a per-request copy of the graph with every grown-touching edge multiplied; original×original cells never move) and `grownHopPenalty` (a DRIFT hop's softmax weight onto a grown topic; `0` keeps drift inside the sixteen, JUMP unaffected).
...
2. **Item pick** — within the chosen topic, weighted random over unseen items above the **curation-score floor** (default 4): `weight = (score − floor + 1)^power × (1 + boost per aesthetic_tag shared with the user's taste keywords)`. Never similarity-ranked — that was the 0.4 failure.
   ...
   > **Sampled pools (09-08-26).** The draw runs over a deterministic per-topic _sample_ of the
   ...
   >
   > **Pools from membership, fetched to plan (09-11-26).** The sample ranks `item_topic` rows joined to their item, so a topic's pool is its whole membership. And a page fetches pools only for the topics its slot draws will land on: `composePage` draws tiers/topics from one random stream and items from another, so `planTopics` can replay the topic sequence without pools; a page that composes short falls back to the two-hop reachable superset. `pickItem` refuses an id already drawn this page, since one item can be in three pools. At today's 104 topics this costs a page ~15-90 ms more than the display-topic query did (p50 176-250 ms against 159-163, under the 300 ms bar); what it buys is that the cost follows the ~30-40 planned topics, not the vocabulary. The query runs in a transaction with `SET LOCAL enable_parallel_hash = off`: a parallel hash join keeps its table in `/dev/shm`, Docker's default 64 MB of it ran out under concurrent pages (SQLSTATE 53100, an intermittent `feed.page` 500), and serial costs a reader with picks nothing measurable. Design and measurements in `docs/DESIGN_feed-on-membership.md`.

   > **Cut 2a note (09-02-26).** Promotion set a display `topic_id` on 2,714 of the 3,741 un-homed items, so the backlog below is now largely drained — 1,027 items remain un-homed and the consequences listed here still apply to exactly those. The feed still reads `item.topic_id`; moving it onto the `item_topic` join (so an item can be drawn under _any_ of its topics, not only its display one) is **Cut 2b**, with its own `bench:feed` run. **Done 09-11-26** — `docs/DESIGN_feed-on-membership.md`; the `topic_edge` half of Cut 2b stays deferred.
   >
   > **Cut 1 note (09-02-26; amended 09-06-26).** `getTopicPools` reads `item_topic` since 09-11-26 (the rail's `drawFromTopic` still reads `item.topic_id`). Consequences for an item that is still un-homed: an **un-homed** item (`topic_id IS NULL`) never enters a _topic_ pool — it reaches the feed only as a **WILD** card (§9, 09-06-26), which is a real page slot but not a topic one; saving one bumps no topic (the toast reads only "Saved to X"); its `/i/` page has no wander teaser and its picture rail is all-wildcard. The gallery's wildcard draw (`drawImageAnywhere`) has no topic filter, so an un-homed _image_ can surface in a wildcard slot — deliberate: it is curated-weighted like every draw, and ignoring the graph is the wildcard's job.
...
**Which tier each draw can reach (Cut 2a, 09-02-26; revised 09-10-26).** CORE draws from `user_topic.weight`, whose rows come from a user picking chips, and the chips are `listTopics()`. Until 09-10-26 that was the sixteen `core` rows, so **CORE could only ever name an original topic**; now `listTopics()` is every _faceted_ topic, so **CORE names whatever the reader actually picked, of either tier** — a reader who picks `sculpture` gets sculpture in their CORE slots. A cold-start user still gets `coldStartWeights()` over the sixteen config ids, which is the one place the original tier is still privileged, and correctly: it is the only set with tuned adjacency to drift from. **DRIFT and JUMP reach the grown tier** as before, by walking the adjacency graph. What that ends is the old asymmetry — "a person picks from a curated sixteen and the corpus's own vocabulary is what they drift _into_" — which was a consequence of the picker's size, not a decision anyone made. One measured consequence still worth watching: with 83 grown topics against 16 original, a sampled 96 cards came back **59 grown / 37 original** _before_ the picker could name a grown topic, so a drift-heavy mix over a 6× vocabulary already spent most of the page outside the reader's picks. That is the backlog becoming reachable working as intended, it is a real change to the feel Ben tuned in Phase 0.5, and it is the open feel question for Cut 2b — now with the picker as a second lever on it.
...
behind `items.galleryRail`, powering the item page's picture — `/g/[itemId]` until 09-10-26). Per slot it rolls a **wildcard** first — with
...
`stay`/`drift`/`jump` mean here exactly what they mean above — with drift and jump _advancing_ the
...
same structural guarantee as `services/wander.ts`, and the reason the rail can serve a public page. It is **not
similarity-ranked**, for the Phase 0.4 reason. And it **writes no `seen_item` rows, ever**: swiping
the rail spends none of the reader's corpus. Repeated `feed.page` draws were rejected in writing
...
**Fonts:** Sora (`next/font/google`, variable — `weight` omitted) for _everything_: headings,
...
`gallery` sheet variant (`--animate-sheet-gallery` — its one user, the gallery's details sheet, went
on 09-10-26; the variant stays in `BottomSheet`), banners, and the install pop-in. All borders
...
- **Playwright (e2e):** ten spec files, 51 tests under `bun run e2e:prod` (09-10-26), covering every flow this section originally listed. `auth.spec.ts` (5.2) — invite gating, sign-up, sign-in, sign-out, and the password-reset round trip through Mailpit's HTTP API. `feed.spec.ts` (5.6) — onboarding → a populated two-column feed, infinite scroll, long-press → item sheet → save, tap → item page → back with `?focus=`, the pill's sheets. `item.spec.ts` (5.7; `gallery.spec.ts` (5.8) folded into it 09-10-26) — the public `/i/[itemId]`, largely signed-out: both variants, the rail driven by the keyboard, Escape back to the intact feed drawing nothing, a rail session spending no corpus, and `/g/`'s redirect. `saved.spec.ts` (5.9), `settings.spec.ts` (5.10), `home.spec.ts` (1.2), `pwa.prod.spec.ts` (5.11), which checks the offline/caching strategy against a real service worker, and `security.spec.ts` (7.2) — the security headers on every kind of route, and zero CSP violations across the real screens under both servers.
...
_Rewritten 09-17-26 as **what is deployed** (8.1 shipped). The plan is `docs/PHASE8_PLAN_8.1.md`; the traps and numbers are `docs/PHASE8_WALKTHROUGH_8.1.md`; the deployed facts table there (resource UUID, volume name, DB container, backup path) is the reference — don't re-derive them from the Coolify UI._

- **Where:** one container on the homelab, VM 202 (`192.168.1.202`), as the second tenant of the Coolify instance (v4.3.x, LAN-only at `:8000`) that also runs ambit-archive. Public at **`https://ambit.benreilly.io`** through the existing `homelab` Cloudflare Tunnel on VM 200 — one ingress rule `service: http://192.168.1.202:3000` (the tunnel's first IP-addressed target; the pre-flight for any future rule is a `curl` from inside the tunnel's network first, see the walkthrough's T4) and a proxied `CNAME ambit → <tunnel-uuid>.cfargotunnel.com`. TLS terminates at Cloudflare's edge; the app learns its own origin from `BETTER_AUTH_URL`, which is what gates HSTS (baked at build) and the `Secure` / `__Secure-` cookie. No Caddy LAN name, no Cloudflare Access (the invite gate is the app's own), no auto-deploy: Coolify's *Deploy* button, because GitHub cannot reach a LAN-only Coolify to deliver a webhook. Not a VPS — the plan text said "VPS or homelab"; the homelab won on cost and on sitting beside the archive it ingests from.
- **The image:** `Dockerfile` — `oven/bun:1.4.0-debian`, full `node_modules` (`drizzle-kit` is what runs migrations), `next build` with `BETTER_AUTH_URL` as the one real build ARG and placeholder `DATABASE_URL` / `BETTER_AUTH_SECRET`; **1.58 GB**, ~3 min to build on the NUC. **Boot = `db:migrate && db:seed && next start`** on every start: the `drizzle/` journal applies, `db:seed` upserts the topic config (facets included — that is how a facet reaches production with nothing copied in), then the app. `/api/health` (`{ok, db, imageCache, commit, ingest, lastIngestAt}`) is the Docker healthcheck, the tunnel's discriminator, and the way to read which commit is live.
...
- **Environment (Coolify, runtime; *Build Variable* ticked only on `BETTER_AUTH_URL`):**
...
- **The volume `ambit-cache` → `/app/.cache`** carries three things and is why a redeploy costs nothing: `img/` (one `<itemId>.webp` per item — **166,137 files / 23 GB** on 09-17-26, filled by `bun run img:warm --rate 2` run detached inside the container, ~22 h for a Tumblr-heavy corpus because the limiter is per host and 144k rows share `64.media.tumblr.com`), `curation/` (798 MB of LLM envelopes — the paid part; the Mac's copy is pushed with `.cache/push-caches.sh` before any walk so production never re-bills), and `pdr/`. **It must stay a volume** (7.3 D3): on an ephemeral filesystem every deploy would send the whole corpus back to the museums. VM root disk is 116 GB LVM; 48 GB free at that count.
- **Nightly ingest is a Coolify Scheduled Task, `bun run ingest` at `30 1 * * *` — and Coolify's cron clock is UTC** (probed 08-30-26 with a `date` task), so it fires at **21:30 EDT / 20:30 EST the evening before**, ahead of the `0 4 * * *` backup — ingest, then back up what it wrote. Its output goes to `/app/.cache/ingest-YYYY-MM-DD.log` on the volume and its timeout is 10800 s. **Coolify's task status is not evidence**: before the timeout was raised every healthy run was recorded `failed` (the 5-minute default killed the bookkeeping while the `docker exec` ran to completion) — verify a run against the database (`max(fetched_at)` clustered in one window) or the dated log, never the UI. A one-off walk (a new blog's budget) is run the same way, detached over `ssh` from the Mac, from a script under `.cache/` — the Coolify runner is not in that path.
- **Cloudflare, two pieces:** the edge does **not** cache HTML (`cf-cache-status: DYNAMIC`, a fresh CSP nonce per request — verified from both sides), and one **Cache Rule** — `(http.host eq "ambit.benreilly.io" and starts_with(http.request.uri.path, "/api/img/"))`, *Eligible for cache* — makes a repeat image request a `HIT` at the edge, so the origin serves each cached WebP roughly once per Cloudflare colo. `cf-connecting-ip` is the client address Better Auth's limiter keys on (per client, not per proxy — proven with two clients and a spoofed `X-Forwarded-For`, walkthrough T6).
- **Operations (8.2, shipped 09-20-26; plan `docs/PHASE8_PLAN_8.2.md`, observed wording in `docs/PHASE8_WALKTHROUGH_8.2.md`).** The alert bus is **Coolify's notifications → Resend → Ben's inbox**, on a second sending-only Resend key named `coolify` (one key per consumer, so either rotates alone) with the shared from-fields `Ambit Ops <ops@ambit.benreilly.io>`. **The alert map:**

  | Event | How it is detected | Channel |
  |---|---|---|
  | An ingest with a dead source | `bun run ingest` exits **2** (`runVerdict`); Coolify _Scheduled Tasks → Failure_ | mail naming the task + exit code (proven: `fail-probe`, minutes) |
  | No successful ingest in 30 h | `/api/health` `"ingest"` ≠ `ok` | UptimeRobot keyword monitor `ambit/ingest`, every 30 min, keyword `"ingest":"ok"` absent → mail |
  | Origin down / tunnel down | `/api/health` non-200 | UptimeRobot HTTP monitor `ambit/health`, every 5 min, from Ashburn, USA |
  | A server-side throw | `instrumentation.ts` → `reportServerError` | one JSON line on stderr always; mail to `OPS_EMAIL` at most once an hour per signature |
  | Deploy failure, container status change, backup failure, server unreachable, disk > 80 % | Coolify's own events | the same Resend channel |
  | Host CPU / memory / disk, container list | Beszel `archive-host` (standalone `docker run` on VM 202, never a Coolify resource) | **visibility only** — no Beszel alert channel exists yet (homelab #34) |

  **The health contract:** `ingest` is `ok` (an `ingest_run` row with `exit_code 0` younger than 30 h), `stale`, `never` (the table is empty) or `unknown` (the read threw), and **it never changes the status code** — `db` and `imageCache` do; that is why the ingest watch is a keyword monitor and not a second status check. Every real run writes a row (dry runs never do; a manual `--source` top-up does, so `ok` means "an ingest succeeded lately", not "the cron fired"). **Standing rules:** every _Success_ notification toggle stays off; the two Resend keys are never shared between consumers; no deploy during an ingest (01:30–02:30 UTC) or a warm; the app container's log driver is Coolify's default `json-file`, `max-size 10m / max-file 3` (30 MB ceiling, read 09-18-26 — nothing to set). **What 8.2 deliberately does not do:** no Sentry/GlitchTip (the trigger is in §15), no Beszel alerting, no push channel, no client-side error reporting (9.5), no invite admin page (9.9).
- **Backups: Coolify's scheduled `pg_dump --format=custom --no-acl --no-owner` of the `ambit` database only, `0 4 * * *` UTC, to local disk under `/data/coolify/backups/databases/root-team-0/ambit-db-<uuid>/pg-dump-ambit-<epoch>.dmp`; the Proxmox nightly `vzdump` of VM 202 carries that file to the NAS.** Retention 14 days / 20 files / 2000 MB. **"Backup All Databases" must stay off**: it switches Coolify to `pg_dumpall`, which needs a superuser to read `pg_authid`, and the app role `ambit` is not one — the result is a 215-byte file holding only the dump header, which Coolify records as a successful backup (found 09-16-26 by the restore drill; every dump on the recreated resource had been empty). **The restore procedure, proven 09-16-26** (8.1 T8: 170,452 items / 2 users / 561,914 memberships restored and matched in 8.5 s, from a 36 MB dump) — on VM 202, with `D` the Postgres 17 container from `docker ps --format '{{.Names}}\t{{.Image}}' | grep postgres:17` and `F` the newest `.dmp`; the copy needs `sudo` because the backups directory is root-only, so run it in a terminal on the VM, never over `bash -s`:
  ```bash
  sudo docker cp "$F" "$D":/tmp/ambit.dmp
  docker exec "$D" psql -U postgres -c 'create database ambit_restore owner ambit'
  docker exec "$D" pg_restore --no-owner --no-acl -U ambit -d ambit_restore /tmp/ambit.dmp
  docker exec "$D" psql -U ambit -d ambit_restore -c 'select count(*) from item' -c 'select count(*) from "user"'
  docker exec "$D" psql -U postgres -c 'drop database ambit_restore'; docker exec "$D" rm -f /tmp/ambit.dmp
  ```
  To restore _for real_, restore into `ambit_restore` the same way, stop the app container, then swap names with `alter database ambit rename to ambit_broken` / `alter database ambit_restore rename to ambit` — never `pg_restore --clean` into the live database. `.cache/restore-drill.sh` and `.cache/restore-drill-cleanup.sh` are the drill as run.
...
- **Visual embeddings → provisional KEEP** — the blind-harness browse read well (image vectors find _form/vibe_ where text finds _subject_). If it ships, it's a "more like this look" save-affordance, not a feed tier — post-MVP; revisit when that affordance is built.
...
- **The curator has no rubric for text items (08-21-26, Phase 6.2)** — `CURATOR_PROMPT` asks for "visually striking or quietly beautiful images" and ideas with "a genuine spark of _huh, I never knew that_". A lyric poem cannot win on either, whatever its quality: Pope and Seeger both scored 4 in the PoetryDB trial, and Wikipedia's corpus-wide 5.27 average has the same cause. Since the prompt is a product artifact carrying Ben's taste calibration, this is his call, not a refactor: either a second rubric branch for `type: "article"`, or an accepted "text scores low and that is what the tier mix is for". **Blocks un-parking `poetrydb`** and is worth settling before 6.3's blogs, which are text items by construction. _Does not bite 6.3's blogs after all (08-27-26) — a link card is image-led and the hero goes to the model as bytes; doorofperception scored 8.64 avg._
...
- **Error tracking beyond one mail an hour (09-20-26, Phase 8.2 D4)** — production has no Sentry/GlitchTip; a server-side throw is one JSON line and one mail per error signature per hour to `OPS_EMAIL`. The trigger for GlitchTip (Postgres 14+, 512 MB, `@sentry/nextjs` with a DSN swap) is either **more than ~5 active users** or **the first time the hourly throttle is what hid a real problem**. Until then the container log (`docker logs`, 30 MB ring) is the second witness.
