...
- `/profile/topics` — the topic manager (09-10-26): four tabs, one facet each, every toggle saved at once via `topics.setMine` (floor of one). Reached from Profile's Topics row and from Settings' "What you see", which was a bottom sheet until this landed. Under `FEED_DEBUG` each pressed chip shows its learned weight and a **Reset weights** button flattens them.
