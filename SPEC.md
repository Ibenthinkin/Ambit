...
| `topic_id` | the item's **display** topic — the seed query that surfaced a museum item, or the first honest topic the curator named for a walk item; `NULL` for an un-homed walk item (Cut 1). Full membership is `item_topic` (§5.1a). The feed's three TOPIC tiers draw on this column until Cut 2b moves them onto the join; `NULL` matches neither `inArray` nor `eq`, so un-homed items are invisible to them with no guard. Since 09-06-26 the **WILD** tier (§9) is the one draw that asks for `NULL` on purpose, so an un-homed item is reachable without being force-fitted into a topic. |
