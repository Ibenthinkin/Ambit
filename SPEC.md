...
CREATE TABLE topic (
  ...
  tier         TEXT NOT NULL DEFAULT 'core' -- 'core' | 'grown'  (Cut 2a, 09-02-26)
);
```

> **`tier` (Cut 2a, 09-02-26).** `core` is the sixteen Ambit shipped with: they are the onboarding chip grid, and they are the rows whose adjacency was tuned by hand in Phase 0.5. `grown` is vocabulary mined from the corpus's own tags and promoted by hand (`scripts/mine-topics.ts` proposes, Ben verdicts, `scripts/promote-topics.ts` applies). Grown topics are fully live in the feed — DRIFT and JUMP reach them and their items are drawn like any other's — but they are deliberately **not** offered as onboarding chips, because a hundred-chip grid is a broken screen. `listTopics()` returns `core` only and backs `topics.list`; `listAllTopics()` is the whole vocabulary. A promoted topic carries `seed_queries {}`: it is vocabulary for classifying walk sources, not a query to send five museum APIs. Onboarding at scale is Cut 3 (`docs/DESIGN_topic-vocabulary-growth.md` §11).
