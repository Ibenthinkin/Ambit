...
- [x] **5.8 — Immersive gallery.** The signature screen, planned in `PHASE5_PLAN_5.8.md` (08-21-26) and built to it. `/g/[itemId]` on `bg-immersive`: a three-cell rail, pointer-event swipe with a 20%-width threshold + `touch-action: none`, chrome hidden by default (600ms unit fade, 10s/10s auto-cycle, inert while hidden), a details sheet on `--animate-sheet-gallery` with drag-close + side-swipe-cycle, and hard-swipe-up / two-finger exits. Pill + both sheets embedded.
  ...
  **The parked archive question, re-grounded and answered as a doorway.** There is no ambit-archive adapter in this repo — "archive items" are labelling support with no rows behind them — so the wish became `wildcardChance` (default **0.1**): the probability a rail slot ignores the topic walk and draws corpus-wide, preferring a `WILDCARD_SOURCES` list that shipped **empty**. A tunable serendipity dial then, and archive items slotted straight into it when that integration landed — which it did the same day: **Phase A.5 added the archive adapter and `"archive"` went into that list**, no other line moving. Client overrides honored only under `FEED_DEBUG`, like `feed.page`'s knobs. Still **no new feed tier and no mechanics** in the feed itself.

  *Done = the full gesture matrix works on iOS Safari (no scroll bleed, no stuck chrome); entry/exit deep-linking correct.* Everything a browser can prove is verified by `e2e/gallery.spec.ts` (three consecutive green runs, 27 tests): cold-open signed-out, tap → chrome → tap-again → details, the article guard, the full feed→item→hero→gallery doorway with the pill's Feed button landing on the intact feed and zero draws in the request log, and a gallery session leaving `seen_item` untouched. **The iOS device pass passed 08-21-26** (over the `tailscale serve` HTTPS origin), and the regression it was watching for did not happen — the hero's long-press still offers "Add to Photos". It found one cluster worth the whole exercise: four gestures across three screens all felt "too hard" for three *shared* reasons — no velocity path on any threshold, an axis re-decided at release rather than locked at the slop, and iOS's `pointercancel` on a claimed multi-touch gesture discarding the two-finger exit at the moment it was recognised. All fixed on this branch (`917c18f`) and pinned by nine new unit tests, though **the fix itself has not been re-confirmed on device** — a threshold is a feel judgement and no test makes one; see the walkthrough. Rail *feel* is deliberately left for a later pass — Ben's read is that it can't be judged honestly until the corpus spans more sources — and `wildcardChance` stays at its untuned 0.1 for the same reason.

- [x] **5.9 — Saved + collections UI.** `/saved` per handoff: title + count line, horizontally scrolling collection chips with live counts (accent-filled active), the shared masonry from 5.6, unsave badge + toast, empty state, image tap → gallery, article tile → reader, pill bookmark filled-white. Share-collection scope needs a call at plan time (public `/c/{collection}` is not in current scope — degrade or defer). Pure UI over 5.5's backend. Also revisit Saved's reachability here (currently two hops from the feed — possibly intentional restraint).

  **Shipped 08-23-26** (`docs/PHASE5_WALKTHROUGH_5.9.md`, plan `docs/PHASE5_PLAN_5.9.md`). Two scope reinterpretations, both this entry's own: chips are **collections** (not the prototype's All/Images/Reading type filter — `saves.list`'s `collectionId` was built for exactly this), and the grid is the **5.6 shared masonry** (not the prototype's rounded caption grid; the whole article tile is the reader tap-target). Filter state lives in the URL; a third `saved-origin` marker (deliberately unabstracted, like `gallery-origin`) makes leaving Saved a history pop — verified zero `feed.page` draws per round trip. Article taps carry no origin marker (feed-origin's semantics would mislabel the item page's pill; the swipe-back seam is commented and accepted). Hydration handoff covers chip taps too: a chip tap is an RSC navigation, so filtered lists ride the route payload with zero client tRPC fetches. **Deferred:** share-collection entirely (no affordance, nothing to degrade); collection creation stays in 5.10; `saves.list` pagination (fine for v1). **Reachability stays open → decide in 5.10** (still two hops from feed; Settings' "Saved with live count" shortcut card is the plan of record).
  *Done = save-from-feed → appears-in-saved → unsave round trip in e2e; chip counts match router integration tests.*

- [x] **5.10 — Profile + Settings + Profile Edit.** **Scope re-baselined with Ben 08-24-26: this entry's original "minimal viable" framing is superseded** — the full designed surface came back into scope, across *three* screens rather than two (the redesign bundle has `Ambit - Profile Edit.dc.html` as well, and all editing lives there). `/profile`: identity row over a **deterministic per-user gradient avatar** (hash of the user id → hue; no storage, no upload, ever in this shape), `@handle` and bio when set, full-width Edit pill, collections grid whose tiles carry **cover images** (most recent saved *image* per collection, additive `cover` on `CollectionWithCount`) and which is led by a dashed **New collection** tile → a name-input sheet (the prototype's zero-input auto-name "Collection 4" was rejected: there is no rename, so an auto-name would be permanent). `/profile/edit`: a dedicated route, not a sheet — name / handle / about, **email read-only** (Better Auth rejects a bare email update by design; change-email is a later auth phase). `/settings`: the whole designed surface, **split real/stub** — real are What you see (topic re-pick), Appearance (the 4-accent knob, now persisted per-device in `localStorage` + an inline `<head>` script for a flash-free reload), Notifications (live browser permission state, with "Unavailable" as a *real* state — the API only exists inside an installed PWA on iOS), Add to home screen, About, Get in touch (mailto), Account details, both shortcut cards, and **Sign out** (its permanent home; `/dev/tokens`'s `sign-out-button.tsx` is deleted, e2e retargeted); stubs are visible rows with real icons that toast "{label} · coming soon" and **display no fabricated values** — the prototype's "2 left"/"Often"/"Not determined" are dropped, though the two stub values that are genuinely true stay ("None" muted sources, "English" language). Backend: two nullable `user` columns (`handle` unique + lowercase-normalized, `bio`), a **`user` tRPC router** (`me`/`updateProfile`) rather than Better Auth's `updateUser` — `getSession` only returns columns Better Auth declares, so `handle`/`bio` are invisible on `ctx.user` and a read path is needed regardless, plus tRPC maps a duplicate handle to a typed `CONFLICT` instead of a 500 — `saves.createCollection`, and `topics.mine`. **Collection deletion/rename deferred entirely** (no design pressure anywhere in the bundle), which keeps the `collections_seeded_at` marker migration out and leaves that note live in `db/collections.ts`. Also answers 5.9's open reachability question: Settings' "Everything kept · N saves" card is Saved's third doorway. Planned cold-executable in `docs/PHASE5_PLAN_5.10.md`.
  *Done = full loop sign-up → onboarding → feed → profile → edit → settings → sign-out in e2e; profile edit round-trips; collection creation + duplicate rejection round-trip.*

  **Shipped 08-24-26** (`docs/PHASE5_WALKTHROUGH_5.10.md`). All fourteen plan decisions held. Four things argued back, none changing the phase's shape: the prototype's warn tint `#D98C6A` **already exists** as `--color-error` (used as `text-error`, not the planned literal); the plan's `useState`+`useEffect` shape for the two client-capability reads trips `react-hooks/set-state-in-effect`, so both became `useSyncExternalStore` with a null `getServerSnapshot` (same pre-mount null, one render sooner, no suppression) while the topics sheet's re-seed took the render-time-adjustment idiom `BottomSheet` already uses; **`listTopics()` carries no `ORDER BY`**, so "What you see" sorts its labels alphabetically rather than deferring to catalog order (**flagged**: the onboarding chip grid renders from that same unordered query, so *its* order is arbitrary too — a real latent bug, left alone as another screen's business); and **global handle uniqueness bit two test fixtures** that used a fixed handle, since e2e user rows persist by design — both are run-scoped now. **`playwright.config.ts` now caps `workers` at 3** (CI: 1): six spec files at Playwright's default five made timeouts rotate between victims across whole-suite runs while every victim passed in isolation — exactly the condition 5.9 said should trigger a worker cap rather than more per-assertion allowances. Three consecutive full-suite runs green afterwards at 1.7–1.8 min, *including* `gallery.spec:193` (CLAUDE.md's documented dev-DB flake — suggestive, not yet proof; the note stays).

- [x] **5.11 — Landing slideshow + install + PWA polish.** Landing per `Landing 2`: full-bleed cross-fading slideshow (Fisher–Yates shuffle per load, `slideMs` cadence, stop-on-last → 260ms → auth sheet rises with the existing password `AuthCard` inside), preload via `new Image()`, orbs + `drift` keyframe deleted. **Hard gate: slide list restricted to the 8 Wikimedia PD works (proxied/cached, not hot-linked) or Ben-cleared images — the bundle's `uploads/*.webp` are licensing-uncleared.** Install: dismissible banner → iOS instruction sheet / real `beforeinstallprompt` elsewhere, dismissal persistence, `pop-in` checkmark; extend `sw.ts` to cache the last feed page (never the personalized feed API responses themselves — deliberate strategy, not `defaultCache`).
  *Done = ✅ Planned 08-24-26 (`docs/PHASE5_PLAN_5.11.md`), executed 08-24/25-26 — walkthrough: `docs/PHASE5_WALKTHROUGH_5.11.md`. 735 vitest tests (93 new), e2e green 40/40 on two of three consecutive runs (run 1's lone failure was the known `gallery.spec:193` dev-DB flake), production build clean, and a new `e2e/pwa.prod.spec.ts` verifying the caching strategy against a real build.*

  **Decisions locked with Ben at plan time:** slides are **the 8 Wikimedia PD works fetched once into `public/landing/` and committed** (not hot-linked — the first screen a reader sees must not depend on Commons' uptime; Ben's own cleared images can be added a line at a time and none had arrived by execution); pacing is **`slideMs` 600 over a random 8-slide subset** (~5s regardless of how long the list grows), with tap-anywhere and a floating glyph to skip; the install banner waits for the **second feed visit**, "Not now" snoozes 30 days, the X is permanent, and it never appears when already standalone.

  **Three findings worth carrying forward.** (1) **`defaultCache` had been caching the personalized feed since Phase 1** — its last rule routes every same-origin `/api/*` except auth through `NetworkFirst` into a shared 16-entry bucket, and tRPC queries travel as GET, so feed pages were in Cache Storage and evicting the image proxy as they went. Replaced by hand-written rules in `src/lib/sw-rules.ts`, unit-tested outside the worker, including the invariant that no tRPC request reaches a caching rule. (2) **An unterminated `runtimeCaching` list silently disables the offline page**: a request matching no rule never enters Serwist's routing, and `fallbacks` only applies to requests Serwist handled — so `sw.ts` ends with a catch-all `NetworkOnly` whose apparent redundancy is the point. Caught only by running the plan's "manual" verification as a script, which is why that script was kept. (3) **`start_url: "/"` defeats an offline launch** — `/` redirects when signed in, and a redirect is what an offline navigation cannot follow; it is now `/feed`, which still bounces signed-out readers server-side.

  **One deliberate prototype deviation:** the "Ambit is on your home screen" confirmation fires on `appinstalled` (Chromium) or on the **first standalone launch** (iOS), never after "Got it" — Safari gives a page no install signal, so confirming on a tap would assert something unknowable.

  **Outstanding, needs a device:** the Chromium install dialog (headless never fires `beforeinstallprompt`) and the iOS home-screen pass over the tailnet. Both listed in the walkthrough's Verification section.

---

## Phase 6 — Learning loop & remaining sources

- [x] **6.1 — Feed learns from saves.** Saves adjust `user_topic.weight` / inferred related topics (SPEC §3.3/§3.4/§9) while retaining the randomness floor. Extend feed tests: a burst of saves in one domain measurably (but not overwhelmingly) shifts composition.
  *Done = ✅ Planned 08-22/23-26 (`docs/PHASE6_PLAN_6.1.md`), executed 08-23-26 — walkthrough: `docs/PHASE6_WALKTHROUGH_6.1.md`. Weight bump covered by the "6.1 — a save teaches the feed" integration block (creation at 1.5, no re-bump on move, cap at 3.0, unsave leaves it); taste-keyword derivation unit- and integration-tested; the seeded distribution block "composePage — learned weights (6.1)" pins both done-bar clauses (a capped topic gains >5pp share but stays under half the page; shipped topicCap bounds it to 3 of 12); the combined save toast says which topic is drifting.*

- [x] **6.2 — Source trials, round 1.** **Reframed at build time (08-21-26).** This step used to read "Remaining v1 adapters (batched)" and name Smithsonian, NASA APOD, Wikiquote and Project Gutenberg/Wikisource — wording that predated the 08-20 correction in `docs/source-candidates.md`, which established that those four were never a committed set but an early draft that outlived its decision. They are *candidates*, and candidates go through the **trial loop**, not a batch add. So 6.2 became the first run of that loop: four candidates, a real adapter and sample ingest each, a written evidence sheet, then a **stop for Ben's Keep/Park/Cut verdicts** before anything was promoted.
  **Trialed:** Smithsonian Open Access · Library of Congress (cleared collections, starting with the Margolies archive) · NASA Image & Video Library · PoetryDB. **NASA Image Library deliberately replaced APOD** — the whole media catalogue with no auth, against one keyed image a day. **Wikiquote and Gutenberg/Wikisource were not in the batch**; they moved to `source-candidates.md` as 🔵 untried rows so dropping them from this line loses nothing.
...
- [x] **9.2 — Accent picker.** ~~Settings surface to choose among the 4 accents~~ — **pulled forward into 5.10**, which had to build the Settings surface anyway. Shipped as a sheet over `lib/accent.ts`, persisted **per-device** in `localStorage` (not a user column: a column costs a migration and a round trip before the app can paint, and a phone and a laptop wanting different accents is reasonable) with an inline `<head>` script re-applying it before first paint. A user column can be added later and seeded from the stored value if cross-device sync ever matters.
...
- [ ] **9.11 — Preference-derived avatar generator.** 5.10 ships a *deterministic* avatar: FNV-1a over the user id → a hue → a two-stop gradient disc (`lib/avatar-hue.ts`). Stable, free, and unique per account, but it says nothing about the person. The backlog item is the richer thing 5.10 explicitly declined to build: a **sprite/glyph mark derived from the reader's own preferences** — picked topics, drift history, most-saved sources — so an avatar becomes a small portrait of what someone reads rather than a hash. Deliberately post-MVP: it needs the taste signal to have accumulated before it means anything, and it must stay non-social (there is no one to show it to but the reader). Also folded in here: giving the **pill toolbar's 25px disc** the per-user gradient, deferred in 5.10 because `PillToolbar` is a presentational primitive with no access to user data.
...
| Image delivery | 7.3 | hotlink vs proxy-with-cache — **6.3 strengthens the proxy case**, and **6.2 strengthened it again 08-21-26**: `tile.loc.gov` rate-limits by IP with no published budget, which is a *cache* problem rather than the referer problem AIC has. Decide together |
| Curator rubric for text items | ⚖️ open | **New 08-21-26 (6.2).** `CURATOR_PROMPT` asks for visually striking images; poems and articles cannot win on it (Pope scored 4; Wikipedia averages 5.27). Ben's call, since the prompt is a taste artifact — blocks un-parking `poetrydb` and lands before 6.3's blogs, which are text by construction |
