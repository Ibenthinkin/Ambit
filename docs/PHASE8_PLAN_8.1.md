...
- [x] **7.4 Image warm (carried item 2)** — task `img-warm`, command `bun run img:warm --rate 2`, **no schedule** (disabled), *Execute now*. Per-host rate, hosts in parallel; ~1–2 h. Then 🖐️ `docker exec "$C" sh -c 'ls .cache/img | wc -l; du -sh .cache/img'` ≈ item count and ~0.7 GB. Two `curl -sI https://ambit.benreilly.io/api/img/<id>` — first `x-ambit-cache: hit` (warmed) and `cf-cache-status: MISS`, second `cf-cache-status: HIT` (T4.5's rule). *Done 08-31-26 for **eight of the nine sources** — archive, cma, doorofperception, loc, met, nasa-images, smithsonian, wellcome are **fully warmed**, and both curl proofs pass on a warmed item: `x-ambit-cache: hit` + `cf-cache-status: MISS`, then `HIT`, `content-length: 94152`, `cache-control: public, max-age=31536000, immutable`. Disk cache, volume and edge rule are all proven. **9,578 files / 909 MB** — over the 0.7 GB the step budgeted, and short of the item count only by wikipedia. The 14 non-wikipedia stragglers resolved into **6 upstream 4xx and 3 undecodable**, which are permanent and correctly never cached (D4: failures are not cached).*
  **The wikipedia half is a code fault, and it is worth reading before you rerun anything.** Two runs both stopped on the same host. Run one (Coolify task `img-warm` #16, `--rate 2`, 14:22→15:42 UTC) filled ~9,100 across the museums and then stopped dead; run two (detached `docker exec`, `--rate 1`, 481 s) filled 434 more and printed the reason the first run had swallowed: `!! upload.wikimedia.org answered 429 3 times in a row after 463 requests — abandoning it for this run.` That is `RATE_LIMIT_GIVE_UP` in `scripts/warm-images.ts` working exactly as designed. **Why only this source:** `wikipedia.ts:118` queries `piprop=original` and line 205 stores `page.original?.source` — the **full-resolution** file. A probe measured one at **11.9 MB**, against museum IIIF derivatives in the low hundreds of KB, so 463 requests pulled multiple GB and wikipedia is the only source that produces `too-big` rejections (9 of them, over `MAX_UPSTREAM_BYTES` = 40 MB). The observed Wikimedia budget is **~450–500 originals per burst**, and lowering `--rate` does not help because the limit is on bytes, not requests. **~846 wikipedia images remain uncached**; those items fall back to live proxy fetches, which is the pre-7.4 status quo for one source in nine. **Do not spend more of Wikimedia's budget on originals the resize throws away — fix the URLs first (7.4c).**
  **Reading the tally, one trap:** the script's `cached` column conflates *already on disk* with *skipped because its host was abandoned* — both increment `tallyRow.skipped`. Run two's `wikipedia … cached 871` is really ~25 cached plus ~846 never attempted. Worth splitting into two counters if the script is ever touched.
  **Coolify was useless here for the same reason as 7.3, and one step worse.** Task #16 is recorded `failed` with `created_at 14:22:27` / `updated_at 14:27:27` — the 5:00 `ScheduledTaskJob` timeout again — while the exec ran on for another 75 minutes, and **the discarded output was the abandon message that would have diagnosed this in one look**. So: **run warms as a detached `docker exec`, never as a Coolify task** — `docker exec -d "$C" sh -c 'bun run img:warm --rate 1 > .cache/img-warm.log 2>&1'` survives the session, is reparented to the container's init, and leaves the tally on the volume where you can read it.
...
- [ ] **7.4c Wikipedia image URLs → 1600px thumbnails, then finish the warm** — the durable fix for 7.4's abandoned host, and it is a code change, so it does not belong in 7.4. `wikipedia.ts` asks PageImages for `piprop=original|name` and stores `page.original?.source`; ask for **`piprop=thumbnail|name&pithumbsize=1600`** and store `page.thumbnail?.source` instead — a `/thumb/` derivative at exactly the size `image-cache.ts` resizes to anyway (`MAX_EDGE = 1600`), so roughly **20× fewer bytes**, no `too-big` losses, and the 429 ceiling stops being reachable. Update `wikipedia.test.ts` and the `WikipediaRaw` / `WikiDetailPage` interfaces (both currently type `original?`). **The 1,320 existing rows do not need a re-ingest** — the transform is mechanical: `/wikipedia/commons/4/41/Name.jpg` → `/wikipedia/commons/thumb/4/41/Name.jpg/1600px-Name.jpg`, so a `renormalize`-shaped script can rewrite them in place (dry-run report first, then `--confirm`, same contract as `renormalize-text.ts`). Then deploy, delete the ~25 wikipedia entries already cached from originals so they refill at the new size, and rerun `docker exec -d "$C" sh -c 'bun run img:warm --source wikipedia --rate 2 > .cache/img-warm.log 2>&1'` — expect ~846 fills, no `too-big`, no abandonment. **If it still 429s, stop and re-measure rather than retrying**; the give-up is protecting a budget, not being timid. *May slip past 8.1's done-bar — nothing else in T7–T9 depends on it, and the affected items degrade to live fetches rather than breaking.*
...
*Done = the feed is populated from a server-side ingest; the cache is warm **for every source but wikipedia, whose ~846 images wait on 7.4c's URL fix**; both survive a redeploy; spend and wall time recorded.*
